...
def test_sync_dify_conversation():
    """测试同步的Dify对话连接"""
    ...
    try:
        ...
        if response1.get('success'):
            print(f"回复内容: {response1.get('reply', 'N/A')[:200]}...")
            ...
            print(f"第一次对话失败: {response1.get('message', 'Unknown error')}")
            return
        ...
        if response2.get('success'):
            print(f"回复内容: {response2.get('reply', 'N/A')[:200]}...")
            ...
            # 检查系统参数是否更新
            updated_system_params = session_service.get_dify_system_params(session_id)
            if updated_system_params:
                dialogue_count = updated_system_params.get('sys.dialogue_count', 0)
                print(f"对话计数: {dialogue_count}")
        else:
            print(f"第二次对话失败: {response2.get('message', 'Unknown error')}")
        ...
        if response3.get('success'):
            print(f"回复内容: {response3.get('reply', 'N/A')[:200]}...")
        else:
            print(f"第三次对话失败: {response3.get('message', 'Unknown error')}")
        ...
        if history_result.get('success'):
            ...
            print(f"对话历史总数: {len(chat_history)}")
            print("所有消息:")
